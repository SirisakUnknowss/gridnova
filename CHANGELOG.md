...
### Fixed
- **Header หน้า Home ล้นออกนอกจอมือถือ**: `.home-header` เป็น flex row ที่ทั้งปุ่มโปรไฟล์และกลุ่ม pill ฝั่งขวาไม่มี `min-width: 0` เลย → หดไม่ได้ พอ hearts pill (1.11.0) เข้ามาแถวก็กว้างเกิน viewport แล้วดันปุ่มเสียงหลุดออกไป · วัดที่ 375px (ชื่อผู้เล่นปกติ + เหรียญ 6 หลัก): scrollWidth 388 / clientWidth 311, ขอบขวาปุ่มเสียงอยู่ที่ x=420 คือ **หลุดนอกจอ 45px** และ `document.scrollWidth` = 420 ทำให้ทั้งหน้าเลื่อนแนวนอนได้ · แก้โดยล็อก pill + ปุ่มเสียงไม่ให้หด (`flex: 0 0 auto`) แล้วให้ชื่อผู้เล่นเป็นตัวที่ตัดด้วย ellipsis แทน + media query ที่ ≤380px บีบ padding ลงอีกชั้น

### Changed
- **เลขเหรียญบน header ย่อแล้ว**: `formatCompact()` — ต่ำกว่า 10,000 โชว์เต็ม, จากนั้นเป็น `12.5K` / `128.5K` / `1.2M` (ตัด boundary ที่ 999,950 เพื่อไม่ให้ออกมาเป็น `1000K`) · ที่อื่น (Profile, Shop, Ledger) ยังโชว์เต็มเหมือนเดิม
- **เอา streak pill ออกจาก top menu** — เหลือ hearts + coins + ปุ่มเสียง ให้แถวไม่แน่น · streak ยังดูได้ที่ Profile และ Calendar
