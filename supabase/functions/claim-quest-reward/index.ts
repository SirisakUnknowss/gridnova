...
Deno.serve(async (req) => {
  ...
  let body: { date: string; quest_id: string };
  try { body = await req.json(); } catch { return reject('INVALID_PAYLOAD'); }

  const { date, quest_id } = body;
  if (!date || !quest_id) return reject('INVALID_PAYLOAD');
  ...
  if (coinReward > 0) {
    await supabaseAdmin.rpc('grant_coins', {
      p_user_id: user.id,
      p_amount: coinReward,
      p_reason: 'quest_claim',
      p_metadata: { quest_id, date },
    });
  }
